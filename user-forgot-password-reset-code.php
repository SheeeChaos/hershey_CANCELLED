...
function send_password_reset($get_username, $get_email, $token)
{
    ...
    try {
        ...
        $mail->Body = 'Here is the verification link <b><a href="http://localhost/hershey/user-change-password.php?token='.$token.'">http://localhost/hershey/user-change-password.php?token='.$token.'</a></b>';
...
if (isset($_POST['password_reset_link'])) {
    ...
    if (mysqli_num_rows($check_email_run) > 0) {
        ...
        if ($update_token_run) {
            ...
            $_SESSION['status'] = "Somethin went wrong";
            header("Location: userforgotpassword.php");
            exit(0);
        }
    } else {
